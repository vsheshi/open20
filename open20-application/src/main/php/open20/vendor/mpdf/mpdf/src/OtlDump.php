		...
		if ($debug) {
			for ($i = 0; $i < count($psName); $i++) {
				...
				if ($oc > 126 || strpos(' [](){}<>/%', $c) !== false) {
					throw new \Mpdf\MpdfException("psName=" . $psName . " contains invalid character " . $c . " ie U+" . ord(l));
				}
			}
		}
		$this->name = $psName;
		...
		if (isset($this->tables["OS/2"])) {
			...
			if ($fsType == 0x0002 || ($fsType & 0x0300) != 0) {
				...
				if (!$overrideTTFFontRestriction) {
					throw new \Mpdf\MpdfException('ERROR - Font file ' . $this->filename . ' cannot be embedded due to copyleft restrictions.');
				}
