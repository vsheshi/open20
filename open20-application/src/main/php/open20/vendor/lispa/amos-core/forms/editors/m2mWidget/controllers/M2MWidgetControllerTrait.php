...
trait M2MWidgetControllerTrait
{
    ...
    public function actionAssociateOneToMany($id)
    {
        ...
        if (Yii::$app->request->getIsPost()) {
            ...
            if (isset(Yii::$app->request->post()['selection'])) {
                $countSelection = count(Yii::$app->request->post()['selection']);
                if ($countSelection == 1) {
                    /** @var ActiveRecord $target */
                    $target = $targetObjClassName::findOne(['id' => Yii::$app->request->post()['selection'][0]]);
                    if (!is_null($target)) {
    ...
    public function actionAssociaM2m($id)
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if (isset($post['selection']) )  {
                ...
                if($save){
                    for ($index = 0; $index < count($post['selection']); $index++) {
                        /** @var ActiveRecord $target */
                        $target = $targetObjClassName::findOne(['id' => Yii::$app->request->post()['selection'][$index]]);
                        if (!is_null($target)) {
    ...
    public function actionAdditionalAssociateM2m($id)
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            $notInTargets = [];
            if (isset(Yii::$app->request->post()['selection'])) {
                for ($index = 0; $index < count(Yii::$app->request->post()['selection']); $index++) {
                    /** @var ActiveRecord $target */
                    $target = $targetObjClassName::findOne(['id' => Yii::$app->request->post()['selection'][$index]]);
                    if (!is_null($target)) {
                        $intercect = $mmTableName::find()->andWhere([$this->mmStartKey => $model->id])
