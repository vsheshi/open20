...
class Generator extends \yii\gii\generators\model\Generator {
    ...
    public function rules() {
        return [
            ...
            [['iconColor', 'iconClass'], 'string'],
            [['tablePrefix', 'widgetScope','widgetFather','moduleName', 'widgetNameInput','widgetDescription','widgetType', 'iconColor', 'iconClass', 'widgetUrl', 'migrationName','rolesSelected', 'widgetVisible', 'iconFramework'], 'safe'],
        ];
    }
...
//pr($this->migrationName, "migrationName");
            ...
            $widgetClassFile = Yii::getAlias('@' . str_replace('\\', '/', $this->ns_4class)) . '/' . $this->widgetName . '.php';
            //$migrationClassFile = Yii::getAlias('@' . str_replace('\\', '/', $this->ns_4migrations)) . '/' . $this->migrationName;
            $migrationClassFile = Yii::getAlias('@' . str_replace('\\', '/', $this->ns_4migrations)) . '/';
...
//pr($migrationClassFile, "path 4 migration");
            ...
            //rations
            $files[] = new CodeFile(
                $migrationClassFile.$this->migration_widget_filename.'.php', $this->render('WidgetMigrationAmosWidget.php', $params)
            );
            $files[] = new CodeFile(
                $migrationClassFile.$this->migration_auth_filename.'.php', $this->render('WidgetMigrationAuthItem.php', $params)
            );
