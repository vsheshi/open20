    ...
    public function rules()
    {
        return array_merge(
            parent::rules(), [
                ...
                [['tablePrefix', 'migrationName'], 'safe'],
            ]
        );
    }
    ...
    public function generate()
    {
        try {
            ...
            foreach ($this->getTableNames() as $tableName) {
                ...
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/base/' . $className . '.php', $this->render('model.php', $params)
                );
                ...
                $ns_migration_model = preg_replace('%[^//]models.*$%', '\migrations', $this->ns);

                $migrationClassFile = Yii::getAlias('@' . str_replace('\\', '/', $ns_migration_model)) . '/' . $this->migrationName;
