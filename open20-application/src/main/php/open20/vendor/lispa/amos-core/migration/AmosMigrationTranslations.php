...
class AmosMigrationTranslations extends Migration
{
    ...
    private function updateLanguageTranslate($languageSourceId, $language, $translation)
    {
        ...
        if (!is_null($languageTranslate)) {
            ...
            MigrationCommon::printConsoleMessage(BaseAmosModule::t('amoscore', 'Language translate not found while updating translation. => languageSourceId') . ': '  . $languageSourceId . '; ' . BaseAmosModule::t('amoscore', 'language') . ': ' . $language . '; ' . BaseAmosModule::t('amoscore', 'translation') . ': ' . $translation);
        }
        return $ok;
    }
    ...
    private function restoreLanguageTranslate($languageSourceId, $language, $translationConf)
    {
        $languageTranslate = $this->findLanguageTranslate($languageSourceId, $language);
        if (is_null($languageTranslate)) {
            MigrationCommon::printConsoleMessage(BaseAmosModule::t('amoscore', 'Language translate not found while restoring translation. => languageSourceId') . ': '  . $languageSourceId . '; ' . BaseAmosModule::t('amoscore', 'language') . ': ' . $language);
            return false;
        }
