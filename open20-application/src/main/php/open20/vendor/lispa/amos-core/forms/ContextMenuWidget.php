...
class ContextMenuWidget extends Widget
{
     ...
     */
    private $mainDivClasses = 'col-xs-2 nop';
    ...
    {
        $modifyTitle = Yii::t('amoscore', 'Modifica');
        $deleteTitle = Yii::t('amoscore', 'Cancella');
        ...
        if (!$this->isDisableModify()) {
            ...
            $buttons[] = Html::a($modifyTitle, $this->getActionModify(), $optionsModify, $checkPermNew);
        }
        if (!$this->isDisableDelete()) {
            $buttons[] = Html::a($deleteTitle, $this->getActionDelete(), $optionsDelete, $checkPermNew);
        }
