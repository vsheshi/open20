...
class RequiredFieldsTipWidget extends Widget
{
    ...
    {
        $tip = Html::beginTag('div', ['class' => 'col-xs-12 note_asterisk nop']);
        $tip .= Html::beginTag('p');
        $tip .= BaseAmosModule::t('amoscore', 'The fields marked with') . ' ';
        $tip .= Html::beginTag('span', ['class' => 'red']) . '*' . Html::endTag('span') . ' ';
        $tip .= BaseAmosModule::t('amoscore', 'are required') . '.';
        $tip .= Html::endTag('p');
        $tip .= Html::endTag('div');
        return $tip;
    }
