...
if ($foldersEnabled) {
    ...
    $columns['updated_by'] = [
        ...
        'value' => function($model){
            $profile = \lispa\amos\admin\models\UserProfile::find()->andWhere(['updated_by' => $model->updated_by])->one();
...
if($controller->documentsModule->enableDocumentVersioning && !$model->is_folder) {
    $columns[] = [
        ...
        'value' => function ($model, $key, $index, $column) {
            \Yii::$app->request->setQueryParams(['parent_id' => $model->id]);
            /** @var  $dataProvider \yii\data\ActiveDataProvider*/
            $modelSearch = new \lispa\amos\documenti\models\search\DocumentiSearch();
            $dataProvider = $modelSearch->searchVersions(\Yii::$app->request->getQueryParams());
