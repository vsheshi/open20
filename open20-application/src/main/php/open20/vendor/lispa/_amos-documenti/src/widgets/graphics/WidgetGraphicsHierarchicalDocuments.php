...
class WidgetGraphicsHierarchicalDocuments extends WidgetGraphic
{
    ...
    public function getNavBar()
    {
        ...
        if (!is_null($this->parent)) {
            ...
            foreach ($parents as $parent) {
                /** @var Documenti $parent */
                $content .= $this->navBarElement($parent);
            }
            $content .= $this->navBarElement($this->parent);
        }
        return $content;
    }
     ...
     * @return string
     */
    private function navBarElement($model = null)
    {
        $url = [
    ...
    {
        $linkOptions = ['href' => '#', 'title' => $model->titolo, 'class' => 'js-pjax'];
        if ($model->is_folder) {
            $href = [
                '/documenti/hierarchical-documents/render-hierarchical-documents-widget',
    ...
    public static function getIconDescription($model)
    {
        ...
        if ($model->is_folder) {
            $countChildren = $model->getChildren()->count();
    ...
    public static function getDocumentDate($model)
    {
        $documentDate = '';
        if (!$model->is_folder) {
            $documentDate = ($model->publicatedFrom ? Html::beginTag('span') . \Yii::$app->formatter->asDate($model->publicatedFrom) . Html::endTag('span') . Html::tag('br')
                : '-');
        }
        return $documentDate;
    }
