...
<div style="border:1px solid #cccccc;padding:10px;margin-bottom: 10px;background-color: #ffffff;margin-top:20px">
    ...
    <div style="box-sizing:border-box;font-size:13px;font-weight:normal;">
        ...
        <?php if (!empty($model_reply)){ ?>
            <p><strong><?=$model_reply->getCreatedUserProfile()->one()->getNomeCognome(). '</strong>' . \Yii::t('app', ' ha risposto ad un commento su una notizia che segui')?></p>
        <?php } elseif ($model->getCreatedUserProfile() != null){?>
            <p><strong><?=$model->getCreatedUserProfile()->one()->getNomeCognome(). '</strong>' . \Yii::t('app', ' ha aggiunto un commento ad una notizia che hai pubblicato')?></p>
        <?php }; ?>
        <p><strong>Notizia: </strong><?= $contextModel->getTitle()?></p>
